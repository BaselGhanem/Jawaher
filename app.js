                    ...
                    <div class="item-card-body">
                        ...
                        <div class="item-sizes mb-3">
                            ${sizes.length === 0 ? `<span style="color:var(--ink-mid);font-size:.8rem">لا توجد مقاسات</span>` : sizes.map(([s, q]) => {
                                ...
                                const vColor= v && v.color ? v.color : '';
                                return `<div style="background:rgba(0,0,0,.02);border:1px solid var(--border);padding:6px;border-radius:8px;margin-bottom:4px;display:flex;justify-content:space-between;align-items:center;width:100%">
                                    <div>
                                        <span style="font-weight:700;font-size:.85rem">${s}</span>: <strong style="${q === 0 ? 'color:var(--ruby)' : ''}">${q}</strong> قطعة
                                        ${vColor ? `<div style="font-size:.7rem;color:var(--ink-mid)"><span style="display:inline-block;width:8px;height:8px;background:${vHex};border-radius:50%;vertical-align:middle;margin-left:4px;border:1px solid #ccc"></span>${vColor}</div>` : ''}
                                    </div>
